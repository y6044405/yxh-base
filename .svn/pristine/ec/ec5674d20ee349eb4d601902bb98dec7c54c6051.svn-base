...
public class VelocityEngineFactory {
    ...
    public VelocityEngine createVelocityEngine() throws IOException, VelocityException {
        ...
        if (!StringUtils.isEmpty(configLocations)) {
            Resource[] resources = new PathMatchingResourcePatternResolver().getResources(configLocations);
            Arrays.sort(resources, new Comparator<Resource>() {
                @Override
                public int compare(Resource o1, Resource o2) {
                    try {
                        int len = o2.getFilename().length() - o1.getFilename().length();
                        return (0 != len)?len:(o2.getURL().getPath().length()) - (o1.getURL().getPath().length());
                    } catch (IOException e) {
                    }
                    return -1;
                }
            ...
            for (Resource resource : resources) {
                logger.info("load velocity config file:{}", resource);
                if (null == prop) {
            ...
            logger.info("  velocity config :{}", prop);
            CollectionUtils.mergePropertiesIntoMap(prop, props);
        }

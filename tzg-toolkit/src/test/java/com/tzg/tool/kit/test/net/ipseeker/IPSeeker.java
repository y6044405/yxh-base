package com.tzg.tool.kit.test.net.ipseeker;
...
import com.tzg.tool.kit.test.net.ipseeker.util.IPEntry;
import com.tzg.tool.kit.test.net.ipseeker.util.IPLocation;
import com.tzg.tool.kit.test.net.ipseeker.util.Message;
import com.tzg.tool.kit.test.net.ipseeker.util.Util;

/**
 * IP地址归属地查询
 * 
 * Filename:    IPSeeker.java  
 * Description:   
 * Copyright:   Copyright (c) 2012-2013 All Rights Reserved.
 * Company:     tzg-soft.com Inc.
 * @author:     flotage
 * @version:    1.0  
 * Create at:   2013-7-30 下午05:31:34  
 *  
 * Modification History:  
 * Date         Author      Version     Description  
 * ------------------------------------------------------------------  
 * 2013-7-30      flotage     1.0         1.0 Version  
 * 
 * 要使用此方法必须把：/data/qqwry.dat 文件拷贝到项目/data/下
 * 
 * 调用说明：
 * IPSeeker.getInstance().getCountry(ips)   获取IP地址所在的地区
 */
public class IPSeeker {
    ...
    private void init(String fileName) {
        ...
        b3 = new byte[3];
        try {
            String file = URLDecoder.decode(this.getClass().getResource(IP_FILE).getFile(), "UTF-8");
    ...
    private String readString(int offset) {
        try {
            ...
            for (i = 0, buf[i] = mbb.get(); buf[i] != 0; buf[++i] = mbb.get())
                ;
            if (i != 0)
                return Util.getString(buf, 0, i, "UTF-8");

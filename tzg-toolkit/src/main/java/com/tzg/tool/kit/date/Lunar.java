 ...
 * 
 * Filename:    Lunar.java  
 * Description:   时间：阳历、阴历、生肖、天干地支
 * Copyright:   Copyright (c) 2012-2013 All Rights Reserved.
 * Company:     tzg-soft.com Inc.
 * @author:     heyiwu 
 * @version:    1.0  
 * Create at:   2012-5-23 下午07:58:59  
 *  
 * Modification History:  
 * Date         Author      Version     Description  
 * ------------------------------------------------------------------  
 * 2012-5-23      heyiwu      1.0         1.0 Version  
 *
 */
public class Lunar {
    private static Logger      logger            = LoggerFactory.getLogger(Lunar.class);
    private int                year;
    private int                month;
    private int                day;
    ...
    final static long[]        lunarInfo         = new long[] { 0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2, 0x04ae0, 0x0a5b6,
            ...
            0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260,
            0x0f263, 0x0d950, 0x05b57, 0x056a0, 0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0,
    ...
    public String getChinaYearString() {
        String sYear = "";
        char[] cYear = ((Integer) year).toString().toCharArray();
        for (int i = 0; i < cYear.length; i++) {
            if (cYear[i] == '0')
                sYear += "零";
            else
                sYear += chineseNumber[Integer.valueOf(((Character) cYear[i]).toString()) - 1];
        }
        return sYear;
    }
    ...
    public String toString() {
        return getChinaYearString() + " 年 " + (leap ? " 闰 " : "") + chineseNumber[month - 1] + " 月 " + getChinaDayString(day) + " 日";
    }
    ...
    public static Calendar setTime(String date) {
        ...
        try {
            today.setTime(new SimpleDateFormat("yyyy-MM-dd").parse(date));
        } catch (ParseException e) {
            logger.error(e.getLocalizedMessage(), e);
        }
        return today;
    ...
    public static void main(String[] args) throws ParseException {
        ...
        //        LoggerFactory.info(chineseDateFormat.parse("1985-10-2").toString());
        logger.info(" 阳历： " + new SimpleDateFormat("yyyy-MM-dd").format(today.getTime()) + " \n 农历:" + lunar);
        logger.info(lunar.animalsYear(lunar.getYear()));
        logger.info(lunar.animalsYear());
        ;
        logger.info(lunar.cyclical());
        logger.info(lunar.cyclical(lunar.getYear()));
        ;
        logger.info(lunar.getDate());
    }
